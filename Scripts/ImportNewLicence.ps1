...
$licence = Get-ChildItem -Path $settings.devLicencePath -Filter '.flf'
Import-NAVServerLicense $settings.serverInstance -LicenseData ([Byte[]]$(Get-Content -Path $licence -Encoding Byte))
